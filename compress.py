...
def compressible_mime(fname):
    t, c = mimetypes.guess_type(fname)
    if not t:
        return False
    if c is not None:
        return "never"
    if t.startswith("text"):
        return True
    if t in COMPRESSIBLE_MIMES:
        return True
    if t.startswith("audio") or t.startswith("video") or t.startswith("image"):
        return "never"
    if t in COMPRESSED_MIMES:
        return "never"
    return True
...
def btrfs_compress(path):
    try:
        ...
        pass
    try:
        p = sp.Popen(["btrfs", "filesystem", "defragment", "-clzo", path])
        p.wait()
    except sp.CalledProcessError:
        pass
